...
def train_lstm(period, interval, ticker = ['AMZN'], hidden_nodes = None):
    ...
    plt.grid()
    plt.suptitle("Learning Curve of " + ticker[0] + " " + period + " period " + interval + " interval " + str(hidden_nodes) + " nodes")
    plt.ylabel("loss")
    plt.xlabel("epochs")
    ...
    plt.title("Predicted vs Actual " + ticker[0] + " " + period + " period " + interval + " interval " + str(hidden_nodes) + " nodes")
    plt.grid()
    plt.ylabel("value")
    plt.xlabel("samples")
    # plt.ylim(top=max(train_scores),bottom=min(train_scores))
